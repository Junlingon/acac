    ...
    return <>
        <div className={`${className}`}>
            ...
            }
            {
                list && list?.map((item) => (
                    <div className={`${className}-item`} key={item.act_id}>
                        ...
                        <div className={`${className}-item-cell`} onClick={() => {
                            linkJump({ link: `https://www.bilibili.com/h5/mall/digital-card/home?-Abrowser=live&act_id=${item.act_id}` })
                        }}>
                            前往助力
                        </div>
                    </div>
            ...
            <div className={`${className}-footer`}>
                {isShowMore ? <div className="btn" onClick={onShowMore}></div> : null }
